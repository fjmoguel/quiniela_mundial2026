...
const CARD_H = 80; // bumped from 70 to fit the bonus row
const CARD_W = 220;
const COL_GAP = 50;
...
export default function BracketView({
  ...
  bonusByMatchNum?: Record<number, { home: number; away: number }>;
  ...
  useEffect(() => { setSlots(bracket); }, [bracket]);
    ...
    <div className="space-y-3 relative">
        ...
        <div className="flex items-center gap-2 flex-wrap">
          <div className="inline-flex rounded-lg border bg-white p-0.5 shadow-sm">
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded transition-all duration-200 ${
                viewMode === "mine" ? "bg-blue-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-50"
              }`}
            >
              📋 Mi bracket
            </button>
            <button
              ...
              className={`px-3 py-1.5 text-xs font-medium rounded transition-all duration-200 ${
                viewMode === "real" ? "bg-green-600 text-white shadow-sm" : "text-gray-600 hover:bg-gray-50"
              }`}
            >
              📊 Real
            </button>
          ...
          {viewMode === "mine" && (
            <span className="text-xs text-gray-500">
              · <span className="text-green-700 font-semibold">+N</span> al lado del equipo = pts ganados
            </span>
          )}
        </div>
      )}
        ...
        >
          {["Dieciseisavos", "Octavos", "Cuartos", "Semis", "Final"].map((title, i) => (
            <div key={title}
              ...
              className="text-xs font-semibold text-gray-600">{title}</div>
          ))}

          <svg style={{ position: "absolute", left: 0, top: 25, pointerEvents: "none" }}
            width={totalWidth} height={TOTAL_H}>
            {Array.from({ length: 8 }).map((_, i) => {
              const y1 = getCenterY("r32", i * 2); const y2 = getCenterY("r32", i * 2 + 1);
              const yMid = getCenterY("r16", i); const x1 = CARD_W; const x2 = ROUND_W;
              return <g key={`r32-r16-${i}`} stroke="#d1d5db" strokeWidth="1.5" fill="none">
                <path d={`M ${x1} ${y1} H ${(x1 + x2) / 2} V ${yMid} H ${x2}`} />
                <path d={`M ${x1} ${y2} H ${(x1 + x2) / 2} V ${yMid}`} /></g>;
            })}
            {Array.from({ length: 4 }).map((_, i) => {
              const y1 = getCenterY("r16", i * 2); const y2 = getCenterY("r16", i * 2 + 1);
              const yMid = getCenterY("qf", i); const x1 = ROUND_W + CARD_W; const x2 = ROUND_W * 2;
              return <g key={`r16-qf-${i}`} stroke="#d1d5db" strokeWidth="1.5" fill="none">
                <path d={`M ${x1} ${y1} H ${(x1 + x2) / 2} V ${yMid} H ${x2}`} />
                <path d={`M ${x1} ${y2} H ${(x1 + x2) / 2} V ${yMid}`} /></g>;
            })}
            {Array.from({ length: 2 }).map((_, i) => {
              const y1 = getCenterY("qf", i * 2); const y2 = getCenterY("qf", i * 2 + 1);
              const yMid = getCenterY("sf", i); const x1 = ROUND_W * 2 + CARD_W; const x2 = ROUND_W * 3;
              return <g key={`qf-sf-${i}`} stroke="#d1d5db" strokeWidth="1.5" fill="none">
                <path d={`M ${x1} ${y1} H ${(x1 + x2) / 2} V ${yMid} H ${x2}`} />
                <path d={`M ${x1} ${y2} H ${(x1 + x2) / 2} V ${yMid}`} /></g>;
            })}
            {(() => {
              const y1 = getCenterY("sf", 0); const y2 = getCenterY("sf", 1);
              const yMid = getCenterY("final", 0); const x1 = ROUND_W * 3 + CARD_W; const x2 = ROUND_W * 4;
              return <g stroke="#facc15" strokeWidth="2" fill="none">
                <path d={`M ${x1} ${y1} H ${(x1 + x2) / 2} V ${yMid} H ${x2}`} />
                <path d={`M ${x1} ${y2} H ${(x1 + x2) / 2} V ${yMid}`} /></g>;
            })()}
          </svg>
          ...
          {r32.map((m, i) => (
            <Positioned key={m.matchNum} slot={m} top={getCenterY("r32", i) - CARD_H / 2 + 25} left={0}
              ...
              bonus={bonusByMatchNum?.[m.matchNum]} matchPoints={m.matchId ? pointsByMatchId?.[m.matchId] : 0} />
          ))}
          {r16.map((m, i) => (
          ...
          {finalSlot && (
            <Positioned slot={finalSlot} top={getCenterY("final", 0) - CARD_H / 2 + 25} left={ROUND_W * 4}
              ...
              bonus={bonusByMatchNum?.[finalSlot.matchNum]} matchPoints={finalSlot.matchId ? pointsByMatchId?.[finalSlot.matchId] : 0} />
          )}
          {thirdSlot && (
            <div style={{ position: "absolute", left: ROUND_W * 4, top: TOTAL_H - CARD_H + 25, width: CARD_W }}>
              <div className="text-[11px] font-semibold text-center mb-1 text-amber-700">🥉 Tercer lugar</div>
              <Card slot={thirdSlot} locked={locked || !showingMine} editing={editingMatchNum === thirdSlot.matchNum}
                ...
                bonus={bonusByMatchNum?.[thirdSlot.matchNum]} matchPoints={thirdSlot.matchId ? pointsByMatchId?.[thirdSlot.matchId] : 0} />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="text-xs text-gray-500 px-1 space-y-1">
        ...
        {hasRealData && showingMine && (
          <p>🎨 <span className="bg-green-200 px-1 rounded">verde fuerte</span> = marcador exacto · <span className="bg-green-50 px-1 rounded border border-green-200">verde claro</span> = ganador correcto · <span className="bg-amber-50 px-1 rounded border border-amber-200">amarillo</span> = teams correctos pero resultado mal · <sup className="text-green-700 font-bold">+N</sup> = pts ganados por ese equipo · 🎯 +N = pts del marcador</p>
        )}
      </div>
    </div>
  );
...
function SaveIndicator({ status, errorMsg }: { status: SaveStatus; errorMsg: string | null }) {
  ...
  let bg = "bg-gray-100 text-gray-700"; let text = "";
  ...
  return <div className={`fixed top-20 right-4 z-50 px-3 py-1.5 rounded-full text-xs font-medium shadow-md ${bg}`}>{text}</div>;
}

function Positioned(props: any) {
  const { slot, top, left, editing, ...rest } = props;
  return <div style={{ position: "absolute", top, left, width: CARD_W, zIndex: editing ? 20 : 1 }}>
    <Card slot={slot} editing={editing} {...rest} /></div>;
}

function comparePrediction(slot: BracketSlot, real: RealMatchData | undefined): {
  bothTeamsMatch: boolean; homeTeamInReal: boolean; awayTeamInReal: boolean;
  exactScore: boolean; correctResult: boolean;
} {
  ...
  const predHome = slot.homeTeamId; const predAway = slot.awayTeamId;
  const realHome = real.homeTeamId; const realAway = real.awayTeamId;
...
function Card({
  slot, locked, editing, onClick, onClose, update, isFinal, real, mode, bonus, matchPoints,
}: any) {
  ...
  if (cmp && cmp.bothTeamsMatch) {
    if (cmp.exactScore) { cardBg = "bg-green-200"; cardBorder = isFinal ? "border-yellow-400 border-2" : "border-green-400 border"; }
    else if (cmp.correctResult) { cardBg = "bg-green-50"; cardBorder = isFinal ? "border-yellow-400 border-2" : "border-green-300 border"; }
    else { cardBg = "bg-amber-50"; cardBorder = isFinal ? "border-yellow-400 border-2" : "border-amber-300 border"; }
  }

  const homeBonus = mode === "mine" ? (bonus?.home ?? 0) : 0;
    ...
    return (
      <div className="border-2 border-blue-400 bg-blue-50 rounded p-2 text-xs shadow-xl" style={{ width: CARD_W + 20, marginLeft: -10 }}>
        ...
        <table className="w-full text-[10px] mb-2">
          ...
          <tbody>
            <tr>
              <td><input type="number" min={0} max={20} value={slot.predHomeScore ?? ""}
                onChange={(e) => update({ predHomeScore: e.target.value === "" ? null : parseInt(e.target.value) })}
                placeholder="-" disabled={locked} className="w-12 px-1 py-0.5 border rounded text-center disabled:bg-gray-100" /></td>
              <td><input type="number" min={0} max={20} value={slot.predHomeScoreET ?? ""}
                onChange={(e) => update({ predHomeScoreET: e.target.value === "" ? null : parseInt(e.target.value) })}
                placeholder="-" disabled={locked} className="w-12 px-1 py-0.5 border rounded text-center disabled:bg-gray-100" /></td>
              <td><input type="number" min={0} max={20} value={slot.predHomePens ?? ""}
                onChange={(e) => update({ predHomePens: e.target.value === "" ? null : parseInt(e.target.value) })}
                placeholder="-" disabled={locked} className="w-12 px-1 py-0.5 border rounded text-center disabled:bg-gray-100" /></td>
            </tr>
            <tr>
              <td><input type="number" min={0} max={20} value={slot.predAwayScore ?? ""}
                onChange={(e) => update({ predAwayScore: e.target.value === "" ? null : parseInt(e.target.value) })}
                placeholder="-" disabled={locked} className="w-12 px-1 py-0.5 border rounded text-center disabled:bg-gray-100" /></td>
              <td><input type="number" min={0} max={20} value={slot.predAwayScoreET ?? ""}
                onChange={(e) => update({ predAwayScoreET: e.target.value === "" ? null : parseInt(e.target.value) })}
                placeholder="-" disabled={locked} className="w-12 px-1 py-0.5 border rounded text-center disabled:bg-gray-100" /></td>
              <td><input type="number" min={0} max={20} value={slot.predAwayPens ?? ""}
                onChange={(e) => update({ predAwayPens: e.target.value === "" ? null : parseInt(e.target.value) })}
                placeholder="-" disabled={locked} className="w-12 px-1 py-0.5 border rounded text-center disabled:bg-gray-100" /></td>
            </tr>
          </tbody>
        </table>
        <div className="text-[9px] text-gray-500">90' = tiempo regular · ET = tras tiempo extra · Pen = penales</div>
      </div>
    );
  }
  ...
  return (
    <button onClick={onClick} disabled={locked && !hasTeams}
      ...
      style={{ height: CARD_H }}>
      <div className="flex items-center justify-between text-[9px] text-gray-400 leading-tight mb-0.5">
        <span>Partido {slot.matchNum}</span>
        {showMatchPts ? (
          <span className="text-green-700 font-bold bg-green-100 border border-green-200 rounded px-1 py-px text-[9px]">
            🎯 +{matchPoints}
          </span>
        ) : null}
      </div>
      ...
      {(slot.predExtraTime || slot.predPenalties) && (
        <div className="text-[9px] text-blue-600 mt-0.5 leading-none">
          {slot.predExtraTime && slot.predHomeScoreET != null ? `ET ${slot.predHomeScoreET}-${slot.predAwayScoreET}` : ""}
          {slot.predExtraTime && slot.predPenalties ? " · " : ""}
          {slot.predPenalties && slot.predHomePens != null ? `Pen ${slot.predHomePens}-${slot.predAwayPens}` : ""}
        </div>
      )}
    </button>
  );
